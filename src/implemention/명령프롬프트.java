...
public class 명령프롬프트 {
    public static void main(String[] args) throws IOException {
        ...
        char[][] filesArr = new char[files][];
        for (int i = 0; i < files; i++)
            filesArr[i] = in.readLine().toCharArray();
        for (int i = 0; i < filesArr[0].length; i++) {
            boolean diff = false;
            for (int j = 1; j < files; j++) {
                if(filesArr[j][i] != filesArr[0][i]){
                    diff= true;
                    break;
                }
            }
            out.write(diff ? "?" : String.valueOf(filesArr[0][i]));
        }
        out.flush();
        in.close();
        out.close();
