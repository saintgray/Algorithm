...
public class Main균형잡힌세상 {
	public static void main(String[] args) throws IOException {
		...
		while(true) {
			...
			charArr = in.readLine().toCharArray();
			for(int i=0; i<charArr.length; i++) {
				
				if(String.valueOf(charArr[i]).matches("[(\\()|(\\))|(\\[)|(\\])]")) {
					if(stack.size()==0) {
						stack.push(charArr[i]);
					}else {
						if(String.valueOf(stack.peek()).concat(String.valueOf(charArr[i])).matches("(\\(\\))|(\\[\\])")) {
							stack.pop();
						}else {
							stack.push(charArr[i]);
						}
					}
				}else if(String.valueOf(charArr[i]).equals(".") && i==charArr.length-1) {
