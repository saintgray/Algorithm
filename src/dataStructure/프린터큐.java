...
public class 프린터큐 {
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < cases; i++) {
			...
			int prints = Integer.parseInt(params[0]);
			int targetIdx = Integer.parseInt(params[1]);

			String[] priorities = in.readLine().split(" ");
			for (int j = 0; j < prints; j++) {
				printQueue.add(Integer.parseInt(priorities[j]));
			}
			
			int printed =0;
			
			boolean isPrintedTarget=false;
			boolean hasPriority= true;
			
			while (!(hasPriority && isPrintedTarget)) {
				
				int printReadyPriority = printQueue.peek();
				hasPriority=true;
				for(int k=printReadyPriority+1; k<=9; k++) {
					if(printQueue.contains(k)) {
						hasPriority=false;
						break;
					}
				}
